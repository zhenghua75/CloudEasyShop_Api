 ...
 */
 internals.loginUser = function(req,reply){
  ...
  var db = req.getDb('CloudEasyShop');
  var User = db.getModel('User');
  //  models.User.find({...}).then({...});
  User.findOne({where:{UserName:req.payload.username}}).then(function(user){
    //console.log(user.get({plain:true}));
    if (_.isNull(user)) {
      reply(Boom.unauthorized('Authentication failed,error username'));      
    }

    // if (Crypto.decrypt(user.PasswordHash) !=
    //     req.payload.password) {
    //   reply(Boom.unauthorized('Authentication failed,error password'));        
    // } else {
    console.log(user.PasswordHash);
    var hashedPassword = Crypto.hashPassword(req.payload.password);
    console.log(hashedPassword);
    console.log(Crypto.verifyHashedPassword(hashedPassword,req.payload.password));
    if(!Crypto.verifyHashedPassword(user.PasswordHash,req.payload.password)){
        reply(Boom.unauthorized('Authentication failed,error password'));  
     } else {
      reply({
        email: user.Email,
    ...
    if (err) {
      console.log(err);
      reply(Boom.unauthorized('Authentication failed,error'));
    }
  })
}
// internals.loginUser = function (req, reply) {
